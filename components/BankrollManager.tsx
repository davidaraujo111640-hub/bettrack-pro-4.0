...
const BankrollManager: React.FC<BankrollManagerProps> = ({ bankrolls, bets, activeBankrollId, onUpdate, onSelect }) => {
  ...
  const [bankToArchive, setBankToArchive] = useState<string | null>(null);
  ...
  const confirmArchive = () => {
    if (!bankToArchive || confirmText.toLowerCase() !== 'archivar') return;
    onUpdate(bankrolls.map(b => b.id === bankToArchive ? { ...b, archived: !b.archived } : b));
  ...
  return (
    <div className="space-y-8 px-4 md:px-0 pb-20">
      ...
      {bankToArchive && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-[#121212] p-10 rounded-[2.5rem] w-full max-w-md border border-white/10 text-center">
            <div className="w-16 h-16 bg-[#ffcc00]/10 rounded-2xl flex items-center justify-center mb-6 mx-auto">
              <Archive size={32} className="text-[#ffcc00]" />
            </div>
            <h3 className="text-xl font-black text-white mb-2 uppercase">¿Archivar Bankroll?</h3>
            <p className="text-slate-500 text-sm font-bold mb-6">
